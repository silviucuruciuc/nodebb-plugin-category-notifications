...
function sendTopicEmail(topic) {
	getSubscribers(topic.cid, topic.user.uid, function(err, uids) {
		...
		var paramsAutoReply = {
			from: 'riskkey.service@aiperion.ai',
			sender: 'riskkey.service@aiperion.ai',
			subject: 'Auto Reply - Message Acknowledged',
			replyTo: 'riskkey.service@aiperion.ai'
		};

		emailer.send('categoryNotifications_autoreply_to_user', topic.user.uid, paramsAutoReply);
