...
function sendTopicEmail(topic) {
	getSubscribers(topic.cid, topic.user.uid, function(err, uids) {
		...
		sendTicketNotificationToRiskkey(topic.user.username, topic.title, topic.category.name);
		sendTicketNotificationToRiskkey2(topic.user.username, topic.title, topic.category.name);
	});
}
...
function sendTicketNotificationToRiskkey(username, topicTitle, categoryName) {
	...
	const endpoint = "http://127.0.0.1:7070" + "/api/email/ticket?username=" + username + "&ticketName=" + topicTitle + "&categoryName=" + categoryName;
	xhttp.open("POST", endpoint, true);
	xhttp.send(null);
}

function sendTicketNotificationToRiskkey2(username, topicTitle, categoryName) {
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function () {
		if (this.readyState == 4 && this.status == 200) {
			winston.info(this.responseText);
	...
	const rmtBaseurl = nconf.get('rmtBaseurl');
	const endpoint = "http://localhost:7070" + "/api/email/ticket?username=" + username + "&ticketName=" + topicTitle + "&categoryName=" + categoryName;
	xhttp.open("POST", endpoint, true);
	xhttp.send(null);
}
